...
def loop_process_frame_queue_for_infer():
    ...
    while (True):
        ...
        processing_frame_cap_source_name = ""
        try:
            frame_queue_lock.acquire()
            if frame_queue:
                # fetch the last one
                processing_frame = frame_queue[-1][0]
                processing_frame_cap_source_name= frame_queue[-1][1]
                if verbose:
                    print('Cycle start at: {}, Fetched a frame(h, w, c): {} from cap source: {}'.format(datetime.datetime.now(),processing_frame.shape,processing_frame_cap_source_name))
                # reduce the q
                frame_queue = []
            else:
                if verbose:
        ...
        try:
            ...
            if total_infer_times % 1 ==0:
                udp_heartbeat_msg = {"sender":local_ip,"msg_type":"heartbeat","cap_source":processing_frame_cap_source_name,"total_infer_times":total_infer_times,"objects":detected_obj_names,"timestamp":str(datetime.datetime.now())}
...
if __name__ == '__main__':
    ...
    while(True):
        try:
            ...
            cap2 = cv2.VideoCapture(FLAGS.input_src_rtsp_uri2)
            print("Is VideoCapture opened: {}".format(cap.isOpened()))
            print("Is VideoCapture2 opened: {}".format(cap2.isOpened()))
            while(cap.isOpened() and cap2.isOpened()):
                if verbose:
                    cap_read_start_time = time.time()
                ret, frame = cap.read()
                if verbose:
                    print('     cap.read() used time: {}ms'.format((time.time() - cap_read_start_time)*1000))
                if not ret:
                    print("!read a broken frame by cap.read(), will re-open the video...")
                    break

                if verbose:
                    cap2_read_start_time = time.time()
                ret2, frame2 = cap2.read()
                if verbose:
                    print('     cap2.read() used time: {}ms'.format((time.time() - cap2_read_start_time)*1000))

                if not ret2:
                    print("!read a broken frame by cap2.read(), will re-open the video...")
                    break
                ...
                try:
                    ...
                    if total_infer_times % 5 ==0:
                        frame_queue.append((frame,"cap"))
                        #frame_queue.append((frame2,"cap2"))
                    else:
                        frame_queue.append((frame2,"cap2"))
                        #frame_queue.append((frame,"cap"))
                finally:
